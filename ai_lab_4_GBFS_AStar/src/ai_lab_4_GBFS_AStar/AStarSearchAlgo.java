...
public class AStarSearchAlgo implements IInformedSearchAlgo {
	...
	public Node execute(Node root, String goal) {
		PriorityQueue<Node> frontier = new PriorityQueue<Node>(new NodeComparatorByHnGn());
			...
			if (currentNode.getLabel().equals(goal))
				...
				for (Edge e : currentNode.getChildren()) {
					...
					if (!explored.contains(e.getEnd()) && !frontier.contains(childNode)) {
						...
						double n1 = childNode.getH() + currentNode.getG() + e.getWeight();
						for (Node nodeInFrontier : frontier) {
							double n2 = nodeInFrontier.getH() + nodeInFrontier.getG();
							if (n1 <= n2) {
								childNode.setG(currentNode.getG() + e.getWeight());
								childNode.setParent(currentNode);
